    ...
    for step_i in range(NUM_STEP):
        ...
        if(total_step +1)% TARGET_UPDATE_INTERVAL == 0:
            for agent_i in range(NUM_AGENT):
                ...
                for target_param, param in zip(agent.tatget_critic.parameters(), agent.critic.parameters()):
                    target_param.data.copy_(agent.tau * param.data + (1.0 - agent.tau) * target_param.data)
                # update target actor 
                for target_param, param in zip(agent.tatget_actor.parameters(), agent.actor.parameters()):
                    target_param.data.copy_(agent.tau * param.data + (1.0 - agent.tau) * target_param.data)
